 ...
 *
 */
public class OBDStartTansFileEventImpl implements OBDEventIble {
    ...
    public void dispose(final byte[] bytes){
        ...
        //阻塞同步
        if(AppData.isNeedSynchronized() && AppData.getCurrentObdCommand().contains(Config.OBD_START_TRANS_FILE)) {
            MyLogger.info("need Synchronzied when send OBD_START_TRANS_FILE command.");
            try {
                AppData.getCarQueue().offer(Config.OBD_START_TRANS_FILE, OBDMethod.TEN_SECONDS_OF_TIME_OUT,
